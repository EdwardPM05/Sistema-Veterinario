<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Especies.aspx.cs" Inherits="VetWeb.Especies" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Gestión de Especies - VetWeb</title>
    <meta charset="utf-8" />
    ...
    <style>
        ...
        *, *::before, *::after {
        ...
        /* Specific IDs for search buttons. Especies.aspx uses btnBuscar and btnLimpiarBusqueda */
        .search-input-group #btnBuscar,
        ...
        .search-input-group #btnLimpiarBusqueda:hover,
    ...
    <form id="form1" runat="server">
        ...
        <nav class="sidebar" id="sidebarMenu">
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item">
                    <a class="nav-link active" data-bs-toggle="collapse" href="#mascotasSubmenu" role="button" aria-expanded="true" aria-controls="mascotasSubmenu">
                        <i class="bi bi-heart-fill"></i><span>Mascotas</span> 
                        <i class="bi bi-chevron-down dropdown-caret"></i>
                    </a>
                    <div class="collapse show" id="mascotasSubmenu"> <%-- 'show' class added to keep it open --%>
                        <ul class="nav flex-column">
                            ...
                            <li class="nav-item"><a class="nav-link active" href="Especies.aspx"><i class="bi bi-tags-fill"></i><span>Especies</span></a></li> <%-- Marked as active --%>
                        </ul>
                    </div>
                </li>
        ...
        <div class="content">
            ...
            <div class="input-group mb-3 search-input-group">
                <asp:TextBox ID="txtBuscarNombreEspecie" runat="server" CssClass="form-control" Placeholder="Buscar por nombre de especie" />
                <asp:Button ID="btnBuscar" runat="server" CssClass="btn btn-outline-secondary" Text="Buscar" OnClick="btnBuscar_Click" />
                <asp:Button ID="btnLimpiarBusqueda" runat="server" CssClass="btn btn-outline-secondary" Text="Limpiar" OnClick="btnLimpiarBusqueda_Click" />
            </div>

            <!-- Button to open the Add/Edit Modal -->
            <button type="button" class="btn btn-custom mb-4" data-bs-toggle="modal" data-bs-target="#especieModal" data-mode="add">
            ...
            <asp:GridView ID="gvEspecies" runat="server" AutoGenerateColumns="False" OnRowCommand="gvEspecies_RowCommand"
                CssClass="table  table-bordered table-hover"
                HeaderStyle-CssClass="table-primary"
                DataKeyNames="EspecieID">
                <Columns>
                    <asp:BoundField DataField="NombreEspecie" HeaderText="Especie" />
                    <asp:TemplateField HeaderText="Acciones" ItemStyle-Width="180px">
                        <ItemTemplate>
                            <asp:Button ID="btnEditar" runat="server" CommandName="Editar" Text="Editar" CssClass="btn btn-primary btn-sm me-2" CommandArgument="<%# Container.DataItemIndex %>" /> <%-- Changed from btn-warning to btn-primary --%>
                            <asp:Button ID="btnEliminar" runat="server" CommandName="Eliminar" Text="Eliminar" CssClass="btn btn-danger btn-sm" CommandArgument="<%# Container.DataItemIndex %>" OnClientClick="return confirm('¿Está seguro de que desea eliminar esta especie?');" />
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </asp:GridView>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <!-- Input field -->
                        <div class="mb-3">
                            <label for="txtNombreEspecie" class="form-label">Nombre de la Especie</label>
                            <asp:TextBox ID="txtNombreEspecie" runat="server" CssClass="form-control" Placeholder="Ej. Canino" />
                        </div>
                        
                        <!-- Hidden Field for EspecieID -->
                        <asp:HiddenField ID="hfEspecieID" runat="server" />
                    </div>
        ...
        <script>
            ...
            // JavaScript function to clear the form fields and set "Add" mode for Especie Modal
            function clearModalFormAndSetAddModeEspecie() { // Renamed for clarity
                // Ensure elements exist before trying to access them
                var txtNombreEspecie = document.getElementById('<%= txtNombreEspecie.ClientID %>');
                var hfEspecieID = document.getElementById('<%= hfEspecieID.ClientID %>');
                ...
                var modalTitle = document.getElementById('especieModalLabel');

                if (txtNombreEspecie) txtNombreEspecie.value = '';
                if (hfEspecieID) hfEspecieID.value = '';
                if (lblMensaje) {
                    lblMensaje.innerHTML = '';
                    lblMensaje.className = '';
                }

                // Show 'Agregar' button, hide 'Actualizar' button
                if (btnAgregar) btnAgregar.style.display = 'inline-block';
                if (btnActualizar) btnActualizar.style.display = 'none';

                // Set modal title
                if (modalTitle) modalTitle.innerText = 'Agregar Nueva Especie';
            }

            // JavaScript function to show the modal (called from C#)
            function showEspecieModal() {
                var myModal = new bootstrap.Modal(document.getElementById('especieModal'));
                myModal.show();
            }

            // JavaScript function to hide the modal (called from C#)
            function hideEspecieModal() {
            ...
            document.addEventListener('DOMContentLoaded', function () {
                var especieModal = document.getElementById('especieModal');
                if (especieModal) {
                    especieModal.addEventListener('shown.bs.modal', function (event) {
                        var button = event.relatedTarget;
                        var isAddModeButton = button && button.getAttribute('data-mode') === 'add';

                        if (isAddModeButton) {
                            clearModalFormAndSetAddModeEspecie(); // Use the renamed function
                        }
                    });
                    especieModal.addEventListener('hidden.bs.modal', function () {
                        clearModalFormAndSetAddModeEspecie(); // Also clear on hide
                    });
                }
            });
        </script>
