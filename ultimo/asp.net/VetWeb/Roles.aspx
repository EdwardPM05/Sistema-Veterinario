<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Roles.aspx.cs" Inherits="VetWeb.Roles" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Gestión de Roles - VetWeb</title>
    <meta charset="utf-8" />
    ...
    <style>
        ...
        .search-input-group #btnBuscarRol { /* Updated ID */
        ...
        .search-input-group #btnBuscarRol:hover { /* Updated ID */
            background-color: #0b5ed7; /* Darker blue on hover */
            border-color: #0b5ed7;
        }

        /* Specific styles for "Limpiar" button */
        .search-input-group #btnLimpiarBusquedaRol { /* Updated ID */
        ...
        .search-input-group #btnLimpiarBusquedaRol:hover { /* Updated ID */
        ...
        #rolModal .alert { /* Changed to rolModal */
        ...
        #rolModal .alert-danger { /* Changed to rolModal */
        ...
        #rolModal .alert-danger::before { /* Changed to rolModal */
        ...
        #rolModal .alert-success::before { /* Changed to rolModal */
        ...
        #rolModal .modal-dialog { /* Changed to rolModal */
    ...
    <form id="form1" runat="server">
        ...
        <nav class="sidebar">
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item"><a class="nav-link active" href="Roles.aspx">Roles</a></li>
        ...
        <div class="content">
            ...
            <div class="input-group mb-3 search-input-group">
                <asp:TextBox ID="txtBuscarNombreRol" runat="server" CssClass="form-control" Placeholder="Buscar por nombre de rol" />
                <asp:Button ID="btnBuscarRol" runat="server" CssClass="btn btn-outline-secondary" Text="Buscar" OnClick="btnBuscarRol_Click" />
                <asp:Button ID="btnLimpiarBusquedaRol" runat="server" CssClass="btn btn-outline-secondary" Text="Limpiar" OnClick="btnLimpiarBusquedaRol_Click" />
            </div>
            ...
            <!-- GridView to display existing roles -->
            <asp:GridView ID="gvRoles" runat="server" AutoGenerateColumns="False" OnRowCommand="gvRoles_RowCommand"
                CssClass="table table-striped table-bordered table-hover"
                HeaderStyle-CssClass="table-primary"
                DataKeyNames="RolID"> <%-- Added DataKeyNames --%>
                <Columns>
                    <%-- Removed RolID column from display --%>
                    <asp:BoundField DataField="NombreRol" HeaderText="Nombre del Rol" />
                    <asp:TemplateField HeaderText="Acciones" ItemStyle-Width="180px">
                        <ItemTemplate>
                            <asp:Button ID="btnEditarRol" runat="server" CommandName="Editar" Text="Editar" CssClass="btn btn-warning btn-sm me-2" CommandArgument="<%# Container.DataItemIndex %>" />
                            <asp:Button ID="btnEliminarRol" runat="server" CommandName="Eliminar" Text="Eliminar" CssClass="btn btn-danger btn-sm" CommandArgument="<%# Container.DataItemIndex %>" OnClientClick="return confirm('¿Está seguro de que desea eliminar este rol?');" />
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </asp:GridView>
        ...
        <!-- Bootstrap Modal for Add/Edit Rol -->
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <!-- Input field for NombreRol -->
                        <div class="mb-3">
                            <label for="txtNombreRol" class="form-label">Nombre del Rol</label>
                            <asp:TextBox ID="txtNombreRol" runat="server" CssClass="form-control" Placeholder="Ej. Administrador" />
                        </div>
                        
                        <!-- Hidden Field for RolID -->
        ...
        <script>
            // JavaScript function to clear the form fields and set "Add" mode for Rol Modal
            function clearModalFormAndSetAddModeRol() {
                var txtNombreRol = document.getElementById('<%= txtNombreRol.ClientID %>');
                var hfRolID = document.getElementById('<%= hfRolID.ClientID %>');
                ...
                if (modalTitle) modalTitle.innerText = 'Agregar Nuevo Rol';
            }

            // JavaScript function to show the Rol modal (called from C#)
            function showRolModal() {
                var myModal = new bootstrap.Modal(document.getElementById('rolModal'));
                myModal.show();
            }

            // JavaScript function to hide the Rol modal (called from C#)
            function hideRolModal() {
                var myModal = bootstrap.Modal.getInstance(document.getElementById('rolModal'));
                if (myModal) {
                    myModal.hide();
                }
            }

            // Event listener for when the Rol modal is shown (for "Agregar" button)
            document.addEventListener('DOMContentLoaded', function () {
                var rolModal = document.getElementById('rolModal');
                if (rolModal) {
                    rolModal.addEventListener('shown.bs.modal', function (event) {
                        var button = event.relatedTarget; 
                        var isAddModeButton = button && button.getAttribute('data-mode') === 'add';

                        if (isAddModeButton) {
                            clearModalFormAndSetAddModeRol();
                        }
                    });
                }
            });
