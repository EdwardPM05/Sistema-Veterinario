<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Servicios.aspx.cs" Inherits="VetWeb.Servicios" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Gestión de Servicios - VetWeb</title>
    <meta charset="utf-8" />
    ...
    *, *::before, *::after {
    ...
    .search-input-group #btnBuscarServicio { background-color: var(--btn-custom-bg); color: white; border-color: var(--btn-custom-bg); border-top-right-radius: 0; border-bottom-right-radius: 0; }
    .search-input-group #btnBuscarServicio:hover { background-color: var(--btn-custom-hover-bg); border-color: var(--btn-custom-hover-bg); }
    .search-input-group #btnLimpiarBusquedaServicio { background-color: var(--btn-clear-bg); color: white; border-color: var(--btn-clear-bg); border-top-right-radius: 8px; border-bottom-right-radius: 8px; }
    .search-input-group #btnLimpiarBusquedaServicio:hover { background-color: var(--btn-clear-hover-bg); border-color: var(--btn-clear-hover-bg); }
    ...
    <form id="form1" runat="server">
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item">
                    ...
                    <div class="collapse show" id="serviciosSubmenu"> <%-- 'show' class added to keep it open --%>
                        <ul class="nav flex-column">
                            <li class="nav-item"><a class="nav-link active" href="Servicios.aspx"><i class="bi bi-tools"></i><span>Ver Servicios</span></a></li> <%-- Marked as active --%>
        ...
        <div class="content">
            ...
            <div class="input-group mb-3 search-input-group">
                <asp:TextBox ID="txtBuscarServicio" runat="server" CssClass="form-control" Placeholder="Buscar por nombre de servicio o subcategoría" />
                <asp:LinkButton ID="btnBuscarServicio" runat="server" OnClick="btnBuscarServicio_Click"
                ...
                <asp:LinkButton ID="btnLimpiarBusquedaServicio" runat="server" OnClick="btnLimpiarBusquedaServicio_Click"
                    CssClass="btn btn-outline-secondary"
                    ToolTip="Limpiar">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                <button type="button" class="btn btn-custom" data-bs-toggle="modal" data-bs-target="#servicioModal" data-mode="add">
                ...
                <div>
                <asp:LinkButton ID="btnImprimirPdfServicios" runat="server" CssClass="btn btn-custom" OnClick="btnImprimirPdf_Click">
            ...
            <!-- GridView para mostrar los servicios existentes -->
            <asp:GridView ID="gvServicios" runat="server" AutoGenerateColumns="False" OnRowCommand="gvServicios_RowCommand"
                CssClass="table table-bordered table-striped table-hover"
                DataKeyNames="ServicioID, SubcategoriaID"> <%-- Claves de datos para una edición robusta --%>
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="Acciones" ItemStyle-Width="120px">
                        <ItemTemplate>
                            <asp:LinkButton ID="btnEditarServicio" runat="server" CommandName="Editar"
                            ...
                            <asp:LinkButton ID="btnEliminarServicio" runat="server" CommandName="Eliminar"
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="servicioModalLabel">Gestión de Servicio</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="txtNombreServicio" class="form-label">Nombre del Servicio</label>
                        ...
                        <div class="mb-3">
                            <label for="ddlSubcategorias" class="form-label">Subcategoría Asociada</label>
                            <asp:DropDownList ID="ddlSubcategorias" runat="server" CssClass="form-select">
                                <%-- Las subcategorías se cargarán desde el code-behind --%>
                            </asp:DropDownList>
                        </div>
                        
                        <!-- Campo oculto para almacenar el ID del servicio al editar -->
        ...
        <script>
            ...
            function clearModalFormAndSetAddModeServicio() {
                var txtNombreServicio = document.getElementById('<%= txtNombreServicio.ClientID %>');
                var txtPrecio = document.getElementById('<%= txtPrecio.ClientID %>');
                var ddlSubcategorias = document.getElementById('<%= ddlSubcategorias.ClientID %>');
                var hfServicioID = document.getElementById('<%= hfServicioID.ClientID %>');
                ...
                var modalTitle = document.getElementById('servicioModalLabel');

                if (txtNombreServicio) txtNombreServicio.value = '';
                if (txtPrecio) txtPrecio.value = '';
                // Restablecer el dropdown a su primera opción ("Ninguna / No aplica")
                if (ddlSubcategorias) ddlSubcategorias.selectedIndex = 0;
                if (hfServicioID) hfServicioID.value = '';
                ...
                if (modalTitle) modalTitle.innerText = 'Agregar Nuevo Servicio';
            }

            // Función JavaScript para mostrar el modal de Servicio (llamada desde C#)
            function showServicioModal() {
                var myModal = new bootstrap.Modal(document.getElementById('servicioModal'));
                myModal.show();
            }

            // Función JavaScript para ocultar el modal de Servicio (llamada desde C#)
            function hideServicioModal() {
                var myModal = bootstrap.Modal.getInstance(document.getElementById('servicioModal'));
                if (myModal) {
                    myModal.hide();
                }
            }

            // Listener de evento para cuando el modal de Servicio se muestra (para manejar el reseteo en modo 'agregar')
            document.addEventListener('DOMContentLoaded', function () {
                var servicioModal = document.getElementById('servicioModal');
                if (servicioModal) {
                    servicioModal.addEventListener('shown.bs.modal', function (event) {
                        ...
                        if (button && button.getAttribute('data-mode') === 'add') {
                            clearModalFormAndSetAddModeServicio();
                        }
                    });
